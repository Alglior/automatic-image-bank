...
def caption_image(client, image_path, id_number):
    try:
        ...
        dense_region_result = client.predict(
            image=handle_file(image_path),
        ...
        dense_region_caption = parse_caption(dense_region_result[0])
...
def save_captions(captions_data, json_path):
    ...
    for filename in os.listdir(folder_path):
        if filename.lower().endswith(('.png', '.jpg', '.jpeg', '.gif')):
            ...
            if image_data:
                ...
                save_captions(captions_data, json_path)
            else:
                print(f"Failed to generate caption for {filename}")

    print("All images have been processed.")
    # Launch another Python script
    subprocess.run(["python", "caption_florence_superflue.py"])
# Usage
